...
不寫檔的替身；比對那一步把 `parity.sync_diff` 換掉 —— 真貨會 rsync 一份安裝版出來，
那是 `tests/integration` 的事。
...
def test_a_broken_comparison_does_not_make_a_successful_publish_look_failed(publish_once):
    """比對炸了（同步腳本跑不完）時什麼都不印，退出碼還是 0，`RESULT_URL` 照樣交出去。

    Doc 已經發出去了，URL 只有此刻拿得到。這支是發佈的附屬品，它自己壞掉不該讓
    `main()` 拋例外 —— 刪掉這條 → 一支壞掉的同步腳本會讓每次發佈都以 traceback 收場。
    """
    def boom(installed, repo):
        raise RuntimeError("rsync 跑不完")

    code, out = publish_once(boom)
