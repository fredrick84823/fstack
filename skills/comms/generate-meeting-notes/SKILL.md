...
| 系列資料夾在 config 裡完全找不到 | 不產，只 DM 給維運者。缺會議類型脈絡，硬產出來的是壞的；也不知道要通知哪個 channel。根目錄有音檔時**不逐檔列**：一個系列一行（系列名＋音檔數），點出它不在設定檔裡 —— 沒有 `meeting_key`，移進日期資料夾照樣產不出來，逐檔喊是執行不了的建議 |
...
| 音檔躺在系列資料夾根目錄（沒進日期資料夾） | 不產 —— 沒有日期資料夾就沒有可信的日期。**已登記的系列走兩軌**：維運者 DM 逐檔列出，該會議的 channel 拿一則「請把它移進 `YYYYMMDD` 資料夾」（身份是 Drive 檔案 id，改名不算新檔案）。**未登記的系列只報系列**，見上一列。非音檔（正式稿、source artifacts、雜檔）不觸發 |
| 任何一場沒產出（跳過或失敗） | **兩軌通知**：維運者 DM 拿例外型別與訊息，該會議的 channel 拿一則「哪一場、為什麼（人話）、已經有人在處理、不用做什麼」的提醒 |
| 同一場、同一個原因每輪都命中 | **一天只發一次**，channel 與 DM 同一套（記在狀態檔的 `notified`）。身份：channel 那則是「場次＋原因」，放錯層那兩則 DM 分別是「有哪幾個檔案」與「有哪幾個系列」—— 同一個未登記系列再多丟一個音檔不算新的一則。「這輪停了」（憑證、掃 Drive、產製失敗）不去重，每輪都講 |
